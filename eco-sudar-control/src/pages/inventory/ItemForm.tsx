...
export default function ItemForm() {
  ...
  useEffect(() => {
    ...
    inventoryApi.rawMaterials()
      .then(all => {
        ...
        if (!m) { toast.error("Item not found"); navigate("/inventory/items"); return; }
        const f: Form = {
          name: m.name,
          unit: m.unit,
  ...
  useUnsavedChanges(dirty && !saving, "item-form");
  ...
  return (
    <FormPage
      ...
      description="Raw material or input consumed in production."
      icon={<Package className="h-6 w-6 text-primary" />}
      onBack={() => navigate("/inventory/items")}
      footer={
        <>
          <Button variant="outline" onClick={() => navigate("/inventory/items")} disabled={saving}>Cancel</Button>
          <Button onClick={submit} disabled={saving || loading}>
            {saving ? "Saving…" : editing ? "Save Item" : "Create Item"}
          </Button>
        </>
      }
    >
